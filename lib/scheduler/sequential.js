    ...
    if (!foundStream) {
      promiseChain = promiseChain.then(function (stream) {
        ...
        if (!stream) {
          ...
          var connectOrTimeoutPromise = self._createConnectOrTimeoutPromise(transportSpecs)
          // ignore errors
          return connectOrTimeoutPromise.catch(function (error) {
...
SequentialScheduler.prototype._createConnectOrTimeoutPromise = function (transportSpecs) {
  ...
  var connectOrTimeoutPromise = myUtils.resolvePromiseOnTimeout(
    connectPromise,
    transport.connectTimeout(),
    onTimeoutP
  )
  return connectOrTimeoutPromise
}
