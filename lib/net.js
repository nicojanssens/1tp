...
var Server = function () {
  ...
  // init
  this._args = merge(Object.create(Server.DEFAULTS), functionArgs.args)
...
Server.prototype._onIncomingConnection = function () {
  ...
  return function (stream, transport, peerConnectionInfo) {
    ...
    var socketArgs = {
      handshake: self._args.handshake
    }
    var socket = new Socket([transport], socketArgs)
    socket.connectStream(stream)
    socket.remoteAddress = peerConnectionInfo
    // if not using curvecp, then execute connection handshake
    if (self._args.handshake) {
    ...
    // otherwise let curvecp take care of this
    } else {
      self._log.debug('firing CONNECTION event -- without executing handshake')
      self.emit('connection', socket)
    }
  }
}
    ...
    this._connectionScheduler.connectP(connectionAttempts)
      .then(function (stream) {
        ...
        // stream is found -- shout it out loud
          ...
          self._log.debug('w00t ... connection established')
          self._log.debug('firing CONNECT event -- without executing handshake')
...
var createServer = function () {
  ...
  return new Server(transports, opts, connectionListener)
}
