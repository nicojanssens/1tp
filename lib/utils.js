...
var resolvePromiseOnTimeout = function (promise, timeout, onTimeoutP) {
  var deferred = Q.defer()
  // execute 'master' promise
  promise.then(deferred.resolve, deferred.reject)
  // when timeout
  delayP(timeout).then(function () {
    // check if there is an onTimeout promise
    if (onTimeoutP) {
      onTimeoutP.then(deferred.resolve, deferred.reject)
    } else {
      deferred.resolve()
    }
  })
...
module.exports.resolvePromiseOnTimeout = resolvePromiseOnTimeout
