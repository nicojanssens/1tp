...
UdpTransport.prototype.listen = function () {
  // parse args
      ...
      if (!handShakeError.message.includes('aborted')) {
        ...
        self.abortP(peerConnectionInfo)
          .then(function () {
            ...
            self._error(new OneTpError(OneTpError.CODES.handshakeAborted, abortCompleteMessage), onFailure)
          })
        ...
        self._log.debug('handshake already aborted -- no further action needed')
        return
      }
    })
}
...
UdpTransport.prototype.abort = function (peerConnectionInfo, onSuccess, onFailure) {
  ...
  if (session === undefined) {
    var noSessionMessage = 'cannot find session for connectionInfo ' + JSON.stringify(peerConnectionInfo)
    this._log.error(noSessionMessage)
    this._error(new OneTpError(OneTpError.CODES.nothingToAbort, noSessionMessage), onFailure)
    return
  }
...
UdpTransport.prototype._onIncomingBytes = function (bytes, rinfo) {
  ...
  var message = signalingFactory.parseTransportPacket(bytes)
...
UdpTransport.prototype._createUdpSession = function (peerAddress, sessionId) {
  ...
  session.once('error', function (error) {
    // delete session
    self._log.error('session ' + session._sessionId + '/' + session._streamId + ' experienced error ' + error)
    self._log.error('removing session ' + session._sessionId)
    delete self._sessions[session._sessionId]
    // test if socket can be closed
    if (myUtils.isEmpty(self._sessions) && self._state === UdpTransport.STATE.CLOSING) {
      self._close(self._onClosingSuccess)
    }
  })
