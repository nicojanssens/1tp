...
UdpTransport.prototype.connect = function (peerConnectionInfo, onSuccess, onFailure) {
  ...
  // and init handshake
  var self = this
  session.initHandshakeP(this._args.connectTimeout)
    .then(function () {
      // fire connect event
    ...
    .catch(function (error) {
      ...
      self.abort(peerConnectionInfo, onFailure)
      // fire error event
      self._error(error, onFailure)
    })
}

UdpTransport.prototype.abort = function (peerConnectionInfo, onFailure) {
  ...
  var session = this._sessions.find(function (session) {
    return session._peerAddress === peerConnectionInfo.transportInfo
  })
  if (!session) {
    var noSessionError = 'cannot find session for connectionInfo ' + JSON.stringify(peerConnectionInfo.transportType)
...
UdpTransport.prototype._onIncomingBytes = function () {
  ...
  return function (bytes, rinfo) {
    ...
    function _onFailure (error) {
      var messageProcessingError = 'could not process message'
    }
