...
var Retry = require('../signaling/retry')
var SignalingMessage = require('../signaling/message')
...
TurnStream.prototype.end = function () {
  ...
  var self = this
  var waitForAck = !this._turnClient._transport.isReliable()
  this._sendSignalingMessage(
    SignalingMessage.TYPE.FIN,
    requestId,
    waitForAck,
    function (error) {
      if (error) {
      ...
      // fire event if no ACKs are expected (otherwise, wait for this ACK)
      if (!waitForAck) {
        self._end()
        // otherwise store handler for later use
      } else {
        self._onAckHandlers[requestId] = self._end.bind(self)
      }
    }
  )
}
  ...
  this._sendSignalingMessage(
    ...
    function (error) {
      ...
      if (!waitForAck) {
        ...
        self._onAckHandlers[requestId] = self._destroy.bind(self)
      }
    }
  )
}

TurnStream.prototype._sendSignalingMessage = function (message, requestId, requiresRetransmission, done) {
  this._log.debug('sending signaling message ' + message + ' with requestId ' + requestId + ' for session ' + this._sessionId + ' to relay address ' + this._peerAddress.relayedAddress.address + ':' + this._peerAddress.relayedAddress.port)
  // prepare data chunk
...
TurnStream.prototype._sendAckMessage = function (requestId) {
  var self = this
  this._sendSignalingMessage(
    SignalingMessage.TYPE.ACK,
    requestId,
    false, // no retransmission of ACKs
    function (error) {
      if (error) {
        self._log.error(error)
        self._error(error)
      } else {
        self._log.debug('outgoing ACK for requestId ' + requestId)
      }
    }
  )
}
...
TurnStream.prototype._sendDataMessage = function (chunk, done) {
  var typeByte = new Buffer(2)
  typeByte.writeUInt16BE(SignalingMessage.TYPE.DATA)
  var sessionIdBytes = netstring.write(this._sessionId)
  var versionBytes = netstring.write(this._version)
...
TurnStream.prototype._processIncomingTurnMessage = function (message, senderAddress) {
  ...
  // if RST packet
  if (message.type === SignalingMessage.TYPE.RST) {
  ...
  // if ACK packet
  if (message.type === SignalingMessage.TYPE.ACK) {
    var requestId = message.data
    this._log.debug('incoming ACK from ' + JSON.stringify(senderAddress) + ' for requestId ' + requestId)
    // when stun transport is unreliable
    if (!this._turnClient._transport.isReliable()) {
      this._retry.confirm(requestId)
      this._onAckHandlers[requestId]()
    }
    return
  }
