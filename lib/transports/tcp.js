...
TcpTransport.prototype.connect = function (peerConnectionInfo, onSuccess, onFailure) {
  ...
  var socket = net.connect(peerConnectionInfo.transportInfo.port, peerConnectionInfo.transportInfo.address, function () {
    self._log.debug('connection established with' + JSON.stringify(peerConnectionInfo))
    // stop timer
...
TcpTransport.prototype.abort = function (peerConnectionInfo, onFailure) {
  ...
  socket.end()
  // delete socket from connectingSockets
  delete this._connectingSockets[peerConnectionInfo.transportInfo]
}
