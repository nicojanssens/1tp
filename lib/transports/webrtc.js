...
WebRtcTransport.prototype.connect = function (peerConnectionInfo, onSuccess, onFailure) {
  ...
  this._signaling.registerP(this._onSignalingMessage.bind(this))
    .then(function (actualRegistrationInfo) {
      ...
      self._sessions[sessionId] = sessionInfo
      ...
      var peer = self._createWebRtcSession(
        self._args,
        peerConnectionInfo.transportInfo,
        sessionInfo
      )
      sessionInfo._peer = peer
      // store peer in case the scheduler wants to abort it
      self._connectingPeers[peerConnectionInfo.transportInfo] = peer
      // add _disconnectFromSignalingServer function for later use
      peer._disconnectFromSignalingServer = function (onDisconnectFailure) {
      ...
      self._setConnectTimeout(
        ...
        self.abort.bind(self, peerConnectionInfo, onFailure),
        onFailure
      )
      ...
      peer.on('error', function () {
        // close connection with signaling server
        peer._disconnectFromSignalingServer(onFailure)
      })
...
WebRtcTransport.prototype._setConnectTimeout = function (peer, onTimeout, onFailure) {
  peer.on('connect', function () {
  ...
  var timeout = setTimeout(function () {
    ...
    onTimeout()
    // fire error event
    self._error(connectionTimeoutError, onFailure)
  }, self._args.connectTimeout)
  peer._1tpConnectionTimeout = timeout
}

WebRtcTransport.prototype.abort = function (peerConnectionInfo, onFailure) {
  ...
  if (!peer) {
    var noPeerError = 'cannot find peer for connectionInfo ' + JSON.stringify(peerConnectionInfo.transportInfo)
    this._log.error(noPeerError)
    this._error(noPeerError, onFailure)
    return
  }
  // destroy peer
  peer.destroy(function () {
    ...
    // close connection with signaling server
    peer._disconnectFromSignalingServer(onFailure)
    // delete peer from connectingPeers
    delete this._connectingPeers[peerConnectionInfo.transportInfo]
  })
}
...
WebRtcTransport.prototype._onSignalingMessage = function (message) {
  ...
  // transport accepts connections + this is an 'init' message (SDP offer)
  if (this._state === WebRtcTransport.STATE.ACTIVE &&
     (data.type === 'offer')
    ) {
...
WebRtcTransport.prototype._createWebRtcSession = function (spArgs, peerTransportInfo, sessionInfo) {
  var peer = new SimplePeer(spArgs)
  var streamId = myUtils.generateStreamId()
  var self = this
  peer.on('signal', this._sendSignalingMessage(peerTransportInfo, sessionInfo))
