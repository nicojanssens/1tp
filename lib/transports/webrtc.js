...
WebRtcTransport.prototype.connect = function (peerConnectionInfo, onSuccess, onFailure) {
  ...
  this._signaling.registerP(this._onSignalingMessage.bind(this))
    .then(function (actualRegistrationInfo) {
      ...
      // when connection-setup aborts
      peer.on('abort', function () {
        // send error message
...
WebRtcTransport.prototype._sendSignalingMessageP = function (signalingDestination, sessionInfo, data) {
  var self = this
...
WebRtcTransport.prototype._createSimplePeer = function (simplePeerArgs, sessionInfo) {
  ...
  var self = this
  var onAbort = this.abort.bind(
    self,
    peerConnectionInfo,
    // on success, let peer fire abort event
    function () {
      peer.emit('abort')
    },
  ...
  peer.on('signal', function (data) {
    console.log('SIGNAL')
    self._sendSignalingMessageP(peerConnectionInfo.transportInfo, sessionInfo, data)
      .then(function () {
        self._log.debug('signaling message ' + JSON.stringify(data) + ' sent successfully')
      })
      .catch(function (error) {
        self._log.error('could not deliver signaling message ' + JSON.stringify(data) + ', error => ' + error)
        // abort handshake
