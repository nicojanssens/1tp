...
WebRtcTransport.prototype._processIncomingSessionRequest = function (message) {
  ...
  // create simple peer
  newSessionInfo._peer = this._createWebRtcSession(
    this._args,
    message.sender,
    newSessionInfo
  )
  // fire connect event on webrtc connect
  var self = this
  newSessionInfo._peer.on('connect', function () {
    // fire connection event
    var peerConnectionInfo = message.sender
    self._fireConnectionEvent(newSessionInfo._peer, self, peerConnectionInfo)
  })
