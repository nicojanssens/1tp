...
WebRtcTransport.prototype.abort = function (peerConnectionInfo, onSuccess, onFailure) {
  ...
  var self = this
  this._getConnectingPeer(peerConnectionInfo, function (peer) {
    if (!peer) {
      var noPeerError = 'cannot find peer for connectionInfo ' + JSON.stringify(peerConnectionInfo.transportInfo)
      self._log.error(noPeerError)
      self._error(noPeerError, onFailure)
      return
    }
...
WebRtcTransport.prototype._getConnectingPeer = function (peerConnectionInfo, onResult, attempts) {
  ...
  var self = this
  setTimeout(function () {
    return self._getConnectingPeer(peerConnectionInfo, onResult, attempts - 1)
  }, 200)
}
