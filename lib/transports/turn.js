...
TurnTransport.prototype.connect = function (peerConnectionInfo, onSuccess, onFailure) {
  ...
  this._signaling.registerP(this._onSignalingMessage.bind(this))
    .then(function (actualRegistrationInfo) {
      ...
      // init turn client
      return sessionInfo.turnClient.initP()
    })
    // then allocate TURN resource
    .then(function () {
      return sessionInfo.turnClient.allocateP()
    })
    // then update signalingInfo and send CONNECT message to peer
    .then(function (allocateReply) {
      self._log.debug('ALLOCATION complete')
      sessionInfo.emit('allocation_complete')
      ...
      // check if connection handshake was not aborted in the meanwhile
      if (sessionInfo.state === 'aborting') {
        throw new Error('connect handshake aborted')
      }
      self._log.debug('sending CONNECT message')
      return self._signaling.sendP(signalingMessage, signalingDestination)
    })
    // then activate READY timeout
    .then(function () {
      ...
      sessionInfo._readyMessageTimeout = setTimeout(function () {
        delete sessionInfo._readyMessageTimeout
        var messageTimeout = 'READY message timeout'
        deferred.reject(messageTimeout)
      }, self._args.connectTimeout)
      // resolve promise once DONE message is transmitted
      sessionInfo.once('done_sent', function () {
        clearTimeout(sessionInfo._readyMessageTimeout)
        delete sessionInfo._readyMessageTimeout
        var readyMsg = 'disabled READY message timeout'
        self._log.debug(readyMsg)
        deferred.resolve()
      })
      // reject promise when session gets aborted
      sessionInfo.once('aborting', function () {
        var timeoutClearedMsg = 'READY message timeout cleared'
        self._log.debug(timeoutClearedMsg)
        deferred.reject(new Error('connect handshake aborted'))
      })
      return deferred.promise
    })
    // then prepare to destroy signaling connection once session is closed
    .then(function () {
      self._log.debug('prepare to destroy signaling connection once session is closed')
      var session = sessionInfo.stream
      session.once('close', function () {
        self._log.debug('session closed, terminating signaling connection')
        // close connection with signaling server
        self._signaling.deregisterP(sessionInfo.myConnectionInfo.transportInfo)
      })
    })
    .catch(function (connectError) {
      // if promised was not rejected due to abort execution, then
      if (connectError.message !== 'connect handshake aborted') {
        // print error
      ...
      sessionInfo.turnClient.closeP()
        .then(function () {
          // remove turn client
          delete sessionInfo.turnClient
          // close connection with signaling server
          self._log.debug('connect failure, terminating signaling connection')
...
TurnTransport.prototype.abort = function (peerConnectionInfo, onSuccess, onFailure) {
  ...
  this._waitUntilTurnClientIsReadyToCloseP(sessionInfo)
    .then(function () {
      sessionInfo.state = 'aborting'
      sessionInfo.emit('aborting')
      self._log.debug('aborting handshake')
      onSuccess()
    })
    .catch(function (error) {
      // handle deregistration error
...
TurnTransport.prototype._close = function () {
  ...
  var self = this
  self._log.debug('socket closed, terminating signaling connection')
  ...
  switch (operationType) {
    case 'connect':
      if (this._acceptIncomingConnections()) {
        this._log.debug('CONNECT message received')
      ...
      break
    case 'ready':
      this._log.debug('READY message received')
      this._onReadyMessage(message)
      break
    case 'done':
      this._log.debug('DONE message received')
...
TurnTransport.prototype._onConnectRequest = function (message) {
  ...
  var self = this
  // fire event
  sessionInfo.emit('connect_received')
  // create turn client and add to sessionInfo
  sessionInfo.turnClient = this._createTurnClient()
  // init turn client
  sessionInfo.turnClient.initP()
    .then(function () {
      // then allocate TURN resource
      return sessionInfo.turnClient.allocateP()
    })
    ...
    .then(function () {
      self._log.debug('PERMISSION created')
      sessionInfo.emit('permission_created')
    ...
    .then(function () {
      ...
      sessionInfo.emit('ready_sent')
      // start timer
      var deferred = Q.defer()
      sessionInfo._doneMessageTimeout = setTimeout(function () {
        delete sessionInfo._doneMessageTimeout
        var messageTimeout = 'DONE message timeout'
        self._log.error(messageTimeout)
        deferred.reject(messageTimeout)
      }, self._args.connectTimeout)
      // resolve promise when DONE message arrives
      sessionInfo.once('done_received', function () {
        clearTimeout(sessionInfo._doneMessageTimeout)
        delete sessionInfo._doneMessageTimeout
        var readyMsg = 'disabled DONE message timeout'
        self._log.debug(readyMsg)
        deferred.resolve()
      })
      return deferred.promise
    ...
    .catch(function (connectError) {
      self._log.debug(connectError)
      // delete session info
      self._deleteTurnSession(sessionInfo)
      // close session's turn client
      sessionInfo.turnClient.closeP()
        .then(function () {
          // remove turn client
          delete sessionInfo.turnClient
...
TurnTransport.prototype._onReadyMessage = function (message) {
  ...
  // fire event
  sessionInfo.emit('ready_received')
  ...
  sessionInfo.turnClient.createPermissionP(connectRequest.relayAddress.address)
    .then(function () {
      ...
      // fire connect event
      self._fireConnectEvent(session, sessionInfo.peerConnectionInfo, sessionInfo._connectOnSuccess)
    ...
    .catch(function (turnError) {
      ...
      sessionInfo.turnClient.closeP()
        .then(function () {
          ...
          delete sessionInfo.turnClient
          // close connection with signaling server
          self._log.debug('error while processing READY message, terminating signaling connection')
          return self._signaling.deregisterP(sessionInfo.myConnectionInfo.transportInfo)
        })
        .then(function () {
...
TurnTransport.prototype._onDoneMessage = function (message) {
  ...
  // fire event
  sessionInfo.emit('done_received')
  ...
  var peerConnectionInfo = {
    ...
    transportInfo: merge(sessionInfo.peerConnectionInfo, sender),
    version: this.version
  }
  this._fireConnectionEvent(sessionInfo.stream, this, peerConnectionInfo)
}
...
TurnTransport.prototype._deleteTurnSession = function (sessionInfo) {
  ...
  if (sessionInfo._refreshTimer !== undefined) {
    clearInterval(sessionInfo._refreshTimer)
    delete sessionInfo._refreshTimer
  }
  if (sessionInfo._createPermissionTimer !== undefined) {
    clearInterval(sessionInfo._createPermissionTimer)
    delete sessionInfo._createPermissionTimer
  }
  if (sessionInfo._channelBindTimer !== undefined) {
    clearInterval(sessionInfo._channelBindTimer)
    delete sessionInfo._channelBindTimer
  }
...
TurnTransport.prototype._waitUntilTurnClientIsReadyToCloseP = function (sessionInfo) {
  ...
  if (sessionInfo.srflxAddress !== undefined) {
    ...
    sessionInfo.once('allocation_complete', function () {
