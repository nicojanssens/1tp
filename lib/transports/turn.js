...
TurnTransport.prototype._onConnectRequest = function (message) {
  ...
  this._turn.allocateP()
    ...
    .then(function () {
      ...
      var session = self._createTurnSession(peerConnectionInfo, self._turn)
...
TurnTransport.prototype._onReadyMessage = function (message) {
  ...
  this._turn.createPermissionP(connectRequest.relayAddress.address)
    .then(function () {
      ...
      var session = self._createTurnSession(peerConnectionInfo, self._turn)
      // fire connect event
      self._fireConnectEvent(session, self._peerConnectionInfo, self._connectOnSuccess)
    })
    .catch(function (error) {
      self._log.error(error)
      self._error(error)
...
TurnTransport.prototype._createTurnSession = function (peerAddress, turnClient) {
  // create new TURN session
  var session = new TurnSession(peerAddress, turnClient)
  // store session
