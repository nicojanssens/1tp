...
TurnTransport.prototype.connect = function (peerConnectionInfo, onSuccess, onFailure) {
  ...
  this._signaling.registerP(this._onSignalingMessage.bind(this))
    .then(function (actualRegistrationInfo) {
      ...
      sessionInfo._turn = self._createTurnClient()
      // execute turn ALLOCATE request
      return sessionInfo._turn.allocateP()
    })
    ...
    .then(function () {
      ...
      sessionInfo._readyMessageTimeout = setTimeout(function () {
        deferred.reject('READY message timeout')
    ...
    .catch(function (connectError) {
      ...
      // close turn client
      sessionInfo._turn.closeP()
        .then(function () {
          // close connection with signaling server
          return self._signaling.deregisterP(sessionInfo._myConnectionInfo.transportInfo)
        })
        .then(function () {
          // delete session info
          self._deleteTurnSession(sessionInfo)
          // fire handshake aborted error
          var handshakeAbortedError = new Error('handshake aborted')
...
TurnTransport.prototype.abort = function (peerConnectionInfo, onSuccess, onFailure) {
  this._log.debug('aborting handshake for ' + JSON.stringify(peerConnectionInfo))
  // verify peerConnectionInfo
  if (peerConnectionInfo.transportType !== this.transportType()) {
    var transportTypeError = 'incorrect connectionInfo: unexpected transportType -- ignoring request'
    this._log.error(transportTypeError)
  ...
  if (!sessionInfo) {
    ...
    this._error(new Error(noSessionInfoMessage), onFailure)
    return
  }
  // close turn client
  var self = this
  this._waitUntilTurnClientIsReadyToCloseP(sessionInfo)
    .then(function () {
      return sessionInfo._turn.closeP()
    })
    .then(function () {
      // close connection with signaling server
      return self._signaling.deregisterP(sessionInfo._myConnectionInfo.transportInfo)
...
TurnTransport.prototype._onConnectRequest = function (message) {
  ...
  var self = this
  // create turn client and add to sessionInfo
  sessionInfo._turn = this._createTurnClient()
  // first allocate TURN resource
  sessionInfo._turn.allocateP()
    .then(function (allocateReply) {
      sessionInfo._srflxAddress = allocateReply.mappedAddress
      sessionInfo._relayAddress = allocateReply.relayedAddress
      if (allocateReply.lifetime !== undefined) {
      ...
      return sessionInfo._turn.createPermissionP(connectRequest.relayAddress.address)
    })
    .then(function () {
      ...
      self._createTurnSession(peerConnectionInfo, sessionInfo._turn, sessionInfo)
      // peer connection info
      sessionInfo._peerConnectionInfo = peerConnectionInfo
    ...
    .then(function () {
      ...
      sessionInfo._doneMessageTimeout = setTimeout(function () {
        deferred.reject('DONE message timeout')
      }, self._args.connectTimeout)
      return deferred.promise
    })
    .catch(function (connectError) {
...
TurnTransport.prototype._onReadyMessage = function (message) {
  ...
  var self = this
  sessionInfo._turn.createPermissionP(connectRequest.relayAddress.address)
    .then(function () {
      // start permission refresh timer
      self._startCreatePermissionTimer(connectRequest.relayAddress.address, sessionInfo)
      // create turn session
      ...
      var session = self._createTurnSession(peerConnectionInfo, sessionInfo._turn, sessionInfo)
      // fire connect event
...
TurnTransport.prototype._startRefreshLoop = function (sessionInfo) {
  ...
  sessionInfo._turn.refreshP(TurnClient.DEFAULT_ALLOCATION_LIFETIME)
...
TurnTransport.prototype._startRefreshTimer = function (lifetime, sessionInfo) {
  ...
  sessionInfo._refreshTimer = setInterval(function () {
    sessionInfo._turn.refreshP(lifetime)
...
TurnTransport.prototype._startCreatePermissionTimer = function (address, sessionInfo) {
  ...
  sessionInfo._createPermissionTimer = setInterval(function () {
    sessionInfo._turn.createPermissionP(address)
      .then(function () {
...
TurnTransport.prototype._startChannelBindTimer = function (address, port, channel, sessionInfo) {
  ...
  sessionInfo._channelBindTimer = setInterval(function () {
    sessionInfo._turn.bindChannelP(address, port, channel)
...
TurnTransport.prototype._waitUntilTurnClientIsReadyToCloseP = function (sessionInfo, attempts) {
  ...
  // resolve if sessionInfo._srflxAddress is defined (indicating that allocate did complete)
  else if (sessionInfo._srflxAddress !== undefined) {
  ...
  // otherwise, try again
    ...
    this._log.debug('turn client not ready to close yet -- waiting ' + timeout + ' ms')
    return Q.delay(timeout).then(function () {
      return self._waitUntilTurnClientIsReadyToCloseP(sessionInfo, attempts - 1)
    })
  }
}
