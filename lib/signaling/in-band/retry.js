...
Retry.prototype.executeP = function (operation, confirmEventName, delay, retries) {
  this._log.debug('scheduling promise execution until receiving event ' + confirmEventName + ', delay = ' + delay + ', retries = ' + retries)
  ...
  return promise.then(function () {
    ...
    if (!confirmed) {
      if (retries === 0) {
        ...
        var newDelay = (self._opts.exponentialBackoff) ? Math.min(self._opts.maxDelay, delay * 2) : delay
        return self.executeP(operation, confirmEventName, newDelay, retries - 1)
      }
    } else {
