...
FilteringUdpTransport.prototype._onIncomingBytes = function (bytes, rinfo) {
  ...
  if (this.filter(message)) {
    console.log('FilteringUdpTransport -- IGNORING message ' + message.type.toString(2) + ' ' + JSON.stringify(message) + ' from ' + JSON.stringify(rinfo))
    // then return -- dropping request on the floor
    return
  }
  // console.log('FilteringUdpTransport -- PROCESSING message ' + message.type + ' ' + JSON.stringify(message) + ' from ' + JSON.stringify(rinfo))
